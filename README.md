...
: 현재 모델은 언어정보점수 부여 시, 중요하다고 판단되는 형태소에 가중치를 주관적인 판단으로 부여하고, 모델 성능 평가를 통해 수정하는 과정을 거쳤다. 하지만 이는 개인에 따라 납득가능한 정도가 다르기 때문에, 수집한 데이터를 활용하여 더 객관적인 언어정보점수를 부여하도록 모델을 수정해야 한다.
