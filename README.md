...
### ▶ 프로젝트 설계 배경
 ...
 국내 정보 탐색 방법의 대부분은 모바일 기기를 통한 웹, 모바일 콘텐츠를 통한 정보 획득 방식이다. 그로 인해, 각 모바일 콘텐츠는 ‘작은 화면을 통해 사용자가 중요한 정보를 얼마나 빠르게 획득할 수 있는지’가 경쟁력이 되었다.
...
(ex. 한국어는 어순이 중요하지 않기 때문에 단어의 시퀀스(순서)에 대한 확률 추정이 효과가 없을 수 있음. 또한, 한국어는 영어와 달리 접사, 조사에 따라 문장성분 또는 품사가 달라질 수 있기 때문에 정확한 문장 분석이 어려움.)
...
- 뉴스 기사의 첫 번째 문장을 '압축 전 원본 문장’으로 하고, 뉴스 기사의 제목을 ‘압축 후 문장’으로 정함.
- 모든 뉴스 기사의 첫 번째 문장과 기사 제목이 연관이 있는 것은 아니기 때문에, 그러한 경우 직접 수정하여 압축 문장을 생성.
...
2. `언어정보점수 계산`
	...
	- 또한, 입력 문장이 뉴스 기사인인 것을 고려하였을 때, 따옴표와 따옴표를 포함하는 문장의 중요도도 높다고 판단하여 해당 토큰에도 점수를 부여함.
```python
# 형태소 분석 후 언어정보점수 추가
...
3. `압축 문장 후보군 생성 (n-gram 생성)`
	- 압축 문장 후보 : 원본 문장에서 특정 어절들을 삭제하여 만든 문장
	- 이를 n-gram을 생성한다고 표현함.
	- 문자열에서 n개의 연속된 요소를 추출한 뒤 삭제하여 압축 문장 후보들을 생성하고, 추출된 n개의 단어 자리에는 ‘[MASK]’라는 단어의 토큰으로 대체.
	- N-gram을 생성할 때에, 적어도 압축 문장 후보 문장들이 문장의 주성분인 주어, 목적어, 보어, 서술어는 최소한 남겨져야한다고 판단하여, 압축 문장 후보의 단어가 4개 이상일 때 동안만 압축 문장 후보 생성을 시행함.
```python
# n-gram
...
4. `Perplexity 계산`
	...
	- Perplexity 값이 작을수록 문장이 문맥적으로 이해 가능한 옳은 문장임을 의미하며, 값이 클수록 문장이 이해하기 어려워 혼잡함을 의미함.
...
### ▶ 프로젝트 결론
 ...
 대체적으로 프로젝트 계획에 맞게 구현이 완료되었지만, 완성된 모델에 아쉬운 점과 개선이 필요한 부분들이 존재하였다. 이 부분에 대해서는 추후에 수정하여 업데이트 할 예정이다.

① n-gram 설정 기준 수정 필요 

: 현재 n-gram의 설정 방식이 너무 직관적이고 단순하다고 생각된다. 문장의 단어가 4개가 남아있을 때까지 압축 문장 후보를 생성하기 때문에 모델을 실행시킬 때 실행 시간이 길어진다. 따라서 더 합리적인 기준으로 압축 문장 후보를 생성하는 방식으로 n-gram 생성에 수정이 필요하다.

② 언어정보점수 부여 시, 객관적인 기준 제시 필요 

: 현재 모델은 언어정보점수 부여 시, 중요하다고 판단되는 형태소에 가중치를 주관적인 판단으로 부여하고, 모델 성능 평가 결과를 참고하여 직접 수정하는 과정을 거쳤다. 하지만 이는 개인에 따라 납득가능한 정도가 다르기 때문에, 수집한 데이터를 활용하여 더 객관적인 언어정보점수를 부여하도록 모델을 수정해야 한다.

③ 모델 성능 평가 중 미해결 오류 

: 모델 성능 평가를 하는 도중, 데이터에 대해서 랜덤으로 발생하는 오류가 존재했다. 하지만, 이에 대해서 해결하지 못하여 오류의 원인을 출력하고 넘어가는 것으로 끝났다. 해당 오류 발생 원인을 찾아 수정해야 한다.

④ 고유명사 인식 해결 

: 한국어로 공개된 개체명 인식기가 존재하지 않아서 문장의 고유명사들을 제대로 판단하는 것에 문제가 있었다. 공개되어 있는 한국어 개체명 인식 관련 데이터를 통해 직접 개체명 인식기를 제작하는 등, 고유명사 인식에 대한 성능을 높이는 방식으로 수정이 필요하다.
